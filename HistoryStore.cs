...
public sealed class HistoryDayCell
{
    ...
    public static HistoryDayCell From(DateTime date, int hour, string? status)
    {
        ...
        var (label, hex) = status switch
        {
            "full" => ("Störung", dark ? "#F87171" : "#B22222"),
            "partial" => ("Einschränkung", dark ? "#FBBF24" : "#FF8C00"),
            "maintenance" => ("Einschränkung", dark ? "#F59E0B" : "#FFA500"),
            "none" => ("OK", dark ? "#34D399" : "#228B22"),
            _ => (status, dark ? "#64748B" : "#CBD5E1")
        };

        return new HistoryDayCell(date, hour, status, $"{tipPrefix}: {label}", BrushFromHex(hex));
    }
    ...
    public static string HexForStatus(string? status)
    {
        ...
        return status switch
        {
            ...
            "maintenance" => dark ? "#F59E0B" : "#FFA500",
